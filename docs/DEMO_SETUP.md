...
│   └── package.json
├── remotes/
│   ├── student-grades/       # Student Grades module (with Zustand)
│   │   ├── src/
...
cd remotes/student-grades && npm install && cd ..
cd remotes/activity-log && npm install && cd ..
cd remotes/image-analyzer && npm install && cd ../..
```
...
2. Click "Student Grades" in navigation
3. Verify student grades list loads
4. Click "Add New Grade"
5. Verify navigation works
6. Click browser back button
7. Verify it works correctly
8. Refresh page on `/student-grades/add`
9. Verify page loads correctly (deep linking)

### Test Remote Standalone Mode

1. Open http://localhost:3105 (Student Grades module standalone)
...
1. In host (http://localhost:3000), navigate to `/student-grades/`
2. Open browser console
3. Check that `useNavigate()` and `useLocation()` hooks work
4. Navigate to `/student-grades/add`
...
3. Navigate to Student Grades module (`/student-grades`)
4. See the same counter value displayed
5. Click "Increment from Student Grades Module" button
6. Counter updates and shows notification
7. Navigate to Activity Log module
8. Click "Decrement from Activity Log" button
...
2. Verify remoteEntry.js is accessible:
   - http://localhost:3105/remoteEntry.js (Student Grades)
   - http://localhost:3106/remoteEntry.js (Activity Log)
   - http://localhost:3107/remoteEntry.js (Image Analyzer)
